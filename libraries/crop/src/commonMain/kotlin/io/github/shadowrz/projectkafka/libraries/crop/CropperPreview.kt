...
fun CropperPreview(
    ...
    val cropRect =
        remember(state.region, viewMat.matrix) {
    ...
    val touchRect =
        remember(cropRect) {
            Rect(
                left = cropRect.left - TOUCH_AREA_OFFSET_PX,
                top = cropRect.top - TOUCH_AREA_OFFSET_PX,
                right = cropRect.right + (TOUCH_AREA_OFFSET_PX * 2),
                bottom = cropRect.bottom + (TOUCH_AREA_OFFSET_PX * 2),
            )
        }
    val zoomLimits =
        remember(state.src.size, view) {
...
fun BringToView(
    ...
    LaunchedEffect(outer, transform) {
        // device rotation
