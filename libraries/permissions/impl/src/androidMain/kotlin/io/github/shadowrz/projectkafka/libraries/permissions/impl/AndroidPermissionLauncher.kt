...
class AndroidPermissionLauncher(val state: PermissionState) : PermissionLauncher {
        ...
        get() =
            when (state.status) {
                is AccompanistPermissionStatus.Denied -> {
                    PermissionStatus.Rejected(shouldShowRationale = state.status.shouldShowRationale)
                }
