...
@Stable
class TopAppBarScrollBehavior internal constructor(internal val scrollBehavior: MaterialTopAppBarScrollBehavior) {
    ...
    /**
     * An optional [androidx.compose.animation.core.AnimationSpec] that defines how the top app bar snaps to either fully collapsed or fully
     * extended state when a fling or a drag scrolled it into an intermediate position.
     */
    val snapAnimationSpec: AnimationSpec<Float>? by scrollBehavior::snapAnimationSpec

    /**
     * An optional [androidx.compose.animation.core.DecayAnimationSpec] that defined how to fling the top app bar when the user flings the
     * app bar itself, or the content below it.
     */
...
@Stable
class TopAppBarState internal constructor(internal val state: MaterialTopAppBarState) {
    /**
    ...
    /**
     * The top app bar's current height offset in pixels. This height offset is applied to the fixed height of the app bar to control the
     * displayed height when content is being scrolled.
     *
     ...
     *
     * This value is updated by a [TopAppBarScrollBehavior] whenever a nested scroll connection consumes scroll events. A common
     * implementation would update the value to be the sum of all [NestedScrollConnection.onPostScroll] `consumed.y` values.
     */
     ...
     *
     * A `0.0` represents a fully expanded bar, and `1.0` represents a fully collapsed bar (computed as [heightOffset] /
     * [heightOffsetLimit]).
     */
     ...
     *
     * A `0.0` indicates that the app bar does not overlap any content, while `1.0` indicates that the entire visible app bar area overlaps
 ...
 *
 * @param initialHeightOffsetLimit the initial value for [TopAppBarState.heightOffsetLimit], which represents the pixel limit that a top app
 *   bar is allowed to collapse when the scrollable content is scrolled
 * @param initialHeightOffset the initial value for [TopAppBarState.heightOffset]. The initial offset height offset should be between zero
 *   and [initialHeightOffsetLimit].
